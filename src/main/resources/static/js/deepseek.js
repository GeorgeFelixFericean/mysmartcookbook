    ...
    return `
      <div class="col-sm-6 col-md-4 col-lg-4 mb-4">
        <div class="card shadow-sm h-100">
          <img src="${recipe.imagePath || '/img/placeholder.jpg'}"
               class="card-img-top"
               alt="${recipe.name}"
		...
		.then(data => {
			...
			if (!data.content?.length) {
				container.innerHTML = `
					<div class="col-12 d-flex justify-content-center mt-4">
						<div class="no-results-message text-center">
							<p>No public recipes match your filters. Try again!</p>
						</div>
					</div>`;
				document.getElementById("paginationContainer").innerHTML = "";
				return;
...
function resetPublicFilters() {
	...
	ingredientContainer.innerHTML = `
		<input type="text" class="form-control mb-2 ingredient-input" placeholder="Enter ingredient name">`;
	...
	try {
		...
		if (!data.content?.length) {
			container.innerHTML = `
				<div class="col-12 d-flex justify-content-center mt-4">
					<div class="no-results-message text-center">
						<p>No public recipes matched your filters. Try again!</p>
					</div>
				</div>`;
			document.getElementById("paginationContainer").innerHTML = "";
			return;
